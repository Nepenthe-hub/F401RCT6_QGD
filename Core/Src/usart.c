...
{
    if (str == NULL) return;
    HAL_UART_Transmit(&huart6, (uint8_t *)str, (uint16_t)strlen(str), 2000);
}

void USART6_SendBuffer(uint8_t *buf, uint16_t len)
{
    if (buf == NULL || len == 0) return;
    HAL_UART_Transmit(&huart6, buf, len, 2000);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART6)
    {
        ...
        HAL_UART_Receive_IT(&huart6, &rx_temp, 1);  /* 重新挂起 */
    }
}
